...
const NoOBSAuthenticator := preload("res://libraries/no-obs-ws/Authenticator.gd")
const NoOBSEnums := preload("res://libraries/no-obs-ws/Utility/Enums.gd")
...
var is_stream_on := true:
	...
	set(val):
		is_stream_on = val
		if is_inside_tree(): scenes_updated.emit()
...
func start(_main : RSMain):
	...
	if RSSettings.obs_autoconnect:
		var has_all := true
		if !RSSettings.obs_websocket_url: has_all = false
		elif !RSSettings.obs_websocket_port: has_all = false
...
func connect_to_obsws(port: int, password: String = "") -> void:
	...
	var err := _ws.connect_to_url("%s:%s" % [RSSettings.obs_websocket_url, port])
	if err == OK:
