...
func sort_by_games(a: RSTwitchUserEntry, b: RSTwitchUserEntry, ascendent: bool) -> bool:
	var a_games: int = a.user.steam_app_ids.size()
	var b_games: int = b.user.steam_app_ids.size()
	if a_games == b_games:
		if a.user.itchio_app_urls.size() == b.user.itchio_app_urls.size():
			return false
		return a.user.itchio_app_urls.size() > b.user.itchio_app_urls.size() == ascendent
