...
func _init(twitch_rest_api: TwitchRestAPI) -> void:
	api = twitch_rest_api;
	fallback_texture = RS.settings.fallback_texture2d;
	_preload_cheemote();
...
func _wait_for_cheeremote(request: BufferedHTTPClient.RequestData, cheer_id: String) -> SpriteFrames:
	...
	var cache_path = RS.settings.cache_cheermote.path_join(cheer_id);
	var sprite_frames = await RS.settings.image_transformer.convert_image(
		cache_path,
