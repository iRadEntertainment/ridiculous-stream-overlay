...
func connect_to_irc() -> void:
	client.connect_to(RS.settings.irc_server_url);
...
func _send_messages() -> void:
	if client.connection_state != WebSocketPeer.STATE_OPEN:
		l.e("Can't send message. Connection not open.")
...
func join_channel(channel_name : String) -> ChannelData:
	if channel_name == "":
		l.e("No channel is specified to join. The channel name can be set on the TwitchIrcChannel node.");
		return;
...
func leave_channel(channel_name : String) -> void:
	if not channel_maps.has(channel_name):
		l.e("Can't leave %s channel cause we are not joined" % channel_name);
		return;
...
func chat(message : String, channel_name : String = ""):
	...
	if channel_name == "":
		l.e("No channel is specified to send %s" % message);
		return;
...
func _handle_message(parsed_message : ParsedMessage) -> void:
	if parsed_message.command != "WHISPER":
		l.i("> [%15s] %s: %s" % [parsed_message.command, parsed_message.server, parsed_message.message]);

	match parsed_message.command:
		"001":
			l.i("Authentication successful.");
			_on_login(true);
...
func _handle_cmd_state(command: String, channel_name: String, tags: Dictionary) -> void:
	...
	l.i("Channel updated %s" % channel_name);
...
func _handle_cmd_notice(info: String) -> bool:
	if info == "Login authentication failed" || info == "Login unsuccessful":
		...
		l.i("No permission. Attempting to obtain new token.");
		await auth.refresh_token();
		if await auth.is_authenticated():
			_login();
			return true;
		else:
			l.i("Please check if you have all required scopes.");
