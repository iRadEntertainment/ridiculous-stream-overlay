...
static func load_profile_image(user: TwitchUser) -> ImageTexture:
	if user == null: return RS.settings.fallback_profile;
	if(ResourceLoader.has_cached(user.profile_image_url)):
		return ResourceLoader.load(user.profile_image_url);
	var client : BufferedHTTPClient = HttpClientManager.get_client(RS.settings.twitch_image_cdn_host);
	...
	if !response.is_empty():
		...
		match content_type:
			...
			_: return RS.settings.fallback_profile;
		texture.set_image(img);
	else:
		# Don't use `texture = RS.settings.fallback_profile` as texture cause the path will be taken over
		# for caching purpose!
		texture.set_image(RS.settings.fallback_profile.get_image());
...
static func shoutout(user: TwitchUser) -> void:
	api.send_a_shoutout(RS.settings.broadcaster_id, user.id, RS.settings.broadcaster_id)
...
static func whisper(_message: String, _username: String) -> void:
