...
var l := RSLogger.new(RSSettings.LOGGER_NAME_EVENT_SUB)
...
var is_connected_to_twitch: bool;
...
func _create_subscriptions_from_config() -> void:
	...
	var _subscriptions: Dictionary = RS.settings.subscriptions;
	for subscription: TwitchSubscriptions.Subscription in _subscriptions:
		var condition: Dictionary = _subscriptions[subscription];
...
func subscribe(data: TwitchCreateEventSubSubscriptionBody):
	...
	if not str(response.response_code).begins_with("2"):
		l.e("Subscription failed for event '%s'. Error %s: %s" % [data.type, response.response_code, response.response_data.get_string_from_utf8()])
		return
	elif (response.response_data.is_empty()):
		return
	l.i("Now listening to '%s' events." % data.type)
...
func _data_received(data : PackedByteArray) -> void:
	...
	if not message_json.has("metadata"):
		l.e("Twitch send something undocumented: %s" % message_str);
		return;
...
func _handle_reconnect(reconnect_message: TwitchReconnectMessage):
	l.i("Session is forced to reconnect");
	...
	l.i("Session reconnected on %s" % reconnect_url);
...
func _message_is_to_old(timestamp: int) -> bool:
	return timestamp < Time.get_unix_time_from_system() - RS.settings.ignore_message_eventsub_in_seconds;
