...
func establish_connection() -> void:
	...
	l.d("Wait %s before connecting" % [wait_time]);
	await Engine.get_main_loop().create_timer(wait_time).timeout;
	l.i("Connecting to %s" % connection_url);
	var err = peer.connect_to_url(connection_url);
	if err != OK:
		l.e("Coulnd't connect to %s cause of %s" % [connection_url, error_string(err)])
	tries+=1;
	connecting = false;
...
func _on_open_connection():
	l.i("connected to %s" % connection_url);
	tries = 0;

func _on_close_connection():
	l.i("connection to %s was closed [%s]: %s" % [connection_url, peer.get_close_code(), peer.get_close_reason()]);
