...
var l := RSLogger.new(RSSettings.LOGGER_NAME_HTTP_SERVER)
...
func start():
	...
	if status != OK:
		l.i("Could not listen to port %d: %s" % [listening_port, error_string(status)]);
...
func _process_request(client: Client) -> void:
	...
	if(peer.get_status() == StreamPeerTCP.STATUS_CONNECTED):
		var error = peer.poll();
		if(error != OK):
			l.e("Can't poll data: %s" % error_string(error))
