...
func is_supported() -> bool:
	if RS.settings.imagemagic_path != "":
		var out = []
		var err = OS.execute(RS.settings.imagemagic_path, ['-version'], out);
		if err == OK:
			converter = ImageMagickConverter.new();
			l.i("Imagemagic detected: %s use it to transform gif/webm" % [ RS.settings.imagemagic_path ]);
			return true;
