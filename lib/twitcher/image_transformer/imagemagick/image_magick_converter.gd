...
var l := RSLogger.new(RSSettings.LOGGER_NAME_IMAGE_LOADER)
...
func _do_work(path: String, buffer: PackedByteArray, output: String, mutex: Mutex) -> SpriteFrames:
	...
	if FileAccess.file_exists(path):
		l.i("File found at %s, loading it instead of using the buffer." % path)
		buffer = FileAccess.get_file_as_bytes(path)
	else:
		DirAccess.make_dir_recursive_absolute(path.get_base_dir());
		var f = FileAccess.open(path, FileAccess.WRITE)
		if f == null:
			l.e("Can't open file %s cause of %s" %[ path, FileAccess.get_open_error()])
...
func _extract_images(file: String, target_folder: String) -> bool:
	...
	if code != 0:
		l.e("unable to convert: %s" % "\n".join(out));
		return false;
	return true;

func _create_fallback_texture():
	var sprite_frames = SpriteFrames.new();
	sprite_frames.add_frame(&"default", RS.settings.fallback_texture2d);
	return sprite_frames;
