...
class_name TwitchGetHypeTrainEvents
	


## 
## #/components/schemas/GetHypeTrainEventsResponse
class Response extends TwitchData:

	## The list of Hype Train events. The list is empty if the broadcaster hasn’t run a Hype Train within the last 5 days.
	@export var data: Array[TwitchHypeTrainEvent]:
		set(val): 
			data = val
			track_data(&"data", val)
	...
	static func create(_data: Array[TwitchHypeTrainEvent]) -> Response:
...
## Contains the information used to page through the list of results. The object is empty if there are no more pages left to page through. [Read More](https://dev.twitch.tv/docs/api/guide#pagination)
## #/components/schemas/GetHypeTrainEventsResponse/Pagination
...
## All optional parameters for TwitchAPI.get_hype_train_events
## #/components/schemas/GetHypeTrainEventsOpt
